...
func TestMem(t *testing.T) {
    doc := gokogiri.XmlParseString("<root>hi<parent><child /><child>Text</child></parent><aunt /><catlady/></root>")
    doc.Free()
...
func TestParallelAddingChildFirst(t *testing.T) {
	testFunc := func(done chan bool) {
		doc := gokogiri.XmlParseString("<root>hi<parent><foo/><bar/></parent></root>")
		done <- false
...
func TestParallelNodeDuplicate(t *testing.T) {
	testFunc := func(done chan bool) {
		doc := gokogiri.XmlParseString("<root><parent><brother>hi</brother></parent></root>")
		done <- false
...
func TestParallelSetContent(t *testing.T) {
	testFunc := func(done chan bool) {
		doc := gokogiri.XmlParseString("<root>hi</root>")
		done <- false
...
func TestParallelPath(t *testing.T) {
	testFunc := func(done chan bool) {
		doc := gokogiri.XmlParseString("<root><child><a/><b/></child></root>")
		done <- false
		child := doc.RootElement().FirstElement()
